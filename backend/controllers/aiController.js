...
exports.getRecommendations = async (req, res) => {
  try {
    ...
    if (req.user) {
      ...
      const recs = campaigns
        .filter(c=>!donated.has(c._id.toString()))
        .filter(c=>!topCat||c.category===topCat||c.isUrgent)
        .slice(0,3);
      return res.json({
        success:true,
        data: recs.length ? recs : campaigns.slice(0,3),
        reason: topCat ? `Based on your ${topCat} donations.` : 'Most urgent campaigns.'
      });
    }
...
exports.getSuggestions = async (req, res) => {
  try {
    ...
    let tiers, reasoning;
    if (campaign.isUrgent || pct < 20) {
      tiers = [1000,2000,5000,10000,20000];
      reasoning = 'Urgent — higher donations have maximum impact.';
    } else if (pct >= 80) {
      const s = Math.max(Math.ceil(remaining/5/100)*100, 100);
      tiers = [s,s*2,s*3,s*5,remaining].map(Math.round);
      reasoning = `Only PKR ${remaining.toLocaleString()} left!`;
    } else {
      tiers = [500,1000,2000,5000,10000];
      reasoning = 'Every amount makes a difference.';
    }
    const impacts = {
      500:   "Provides a family's daily meal.",
      1000:  "Covers a week's clean water.",
      2000:  "Funds medicines for a child.",
      5000:  "Emergency shelter for a family.",
      10000: "Sponsors a child's education.",
      20000: "Funds a complete medical kit."
    };
...
exports.generateContent = async (req, res) => {
  ...
  try {
    ...
    const prompt = `You are an AI for NASEER, a Palestinian humanitarian donation platform.
Generate campaign content for: "${topic}" with PKR ${amount||1000}.
...
{"title":"max 8 words","description":"2-3 sentences","impact":"one sentence what PKR ${amount||1000} provides","urduTagline":"short urdu tagline","callToAction":"max 6 words"}`;
    ...
    res.json({ success:true, fallback:true, data:{
      title:       `Support Palestine: ${topic}`,
      description: 'Critical humanitarian aid for Palestinian families in desperate need.',
      impact:      `PKR ${amount||1000} feeds a family for three days.`,
      urduTagline: 'آپ کی مدد فلسطین کو طاقت دیتی ہے',
      callToAction:'Donate Now, Save Lives'
    }});
  }
};
...
exports.getImpact = async (req, res) => {
  try {
    ...
    const result = await model.generateContent(
      `One sentence under 20 words starting "PKR X can..." describing the impact of donating to: ${campaign.title}. Only the sentence, no quotes.`
    );
